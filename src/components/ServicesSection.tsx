...
const ServicesSection = () => {
  ...
  const services: Service[] = [
    {
      title: "טיפולי פנים",
      image: price1, // דוגמה לנתיב תמונה
      items: [
        {
  ...
  return (
    <motion.div 
      className="max-w-4xl mx-auto p-6 bg-amber-200 rounded-lg shadow-md"
      initial="hidden"
      animate="visible"
      ...
      <AnimatePresence>
        {services.map((service, index) => (
          <motion.div
            key={service.title}
            variants={itemVariants}
            className={`mb-10 ${index !== services.length - 1 ? 'border-b border-gray-200' : ''}`}
          >
            <div className="flex flex-col md:flex-row gap-6 items-start">
              ...
              <div className="flex-1">
                <motion.h3 
                  className="text-2xl font-semibold mb-4 text-primary-600 cursor-pointer"
                ...
                >
                  {service.items.map((item) => (
                    <motion.div
                      ...
                      className="flex justify-between items-center bg-amber-100 p-4 rounded-lg hover:bg-gray-100"
                      whileHover={{ x: 5 }}
                    >
